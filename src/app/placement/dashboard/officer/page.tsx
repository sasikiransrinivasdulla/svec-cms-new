...
export default function PlacementOfficerPage() {
    ...
    const compressImage = (file: File, maxSizeKB: number = 300, quality: number = 0.8): Promise<File> => {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (fileSizeKB > maxSizeKB) {
                toast.info('Image is being compressed...');
                finalFile = await compressImage(file, maxSizeKB, 0.8);
                
                // Check if compressed file is still too large
                const compressedSizeKB = finalFile.size / 1024;
                if (compressedSizeKB > maxSizeKB) {
                    // Try with lower quality
                    finalFile = await compressImage(file, maxSizeKB, 0.6);
                    const finalSizeKB = finalFile.size / 1024;
                    
                    if (finalSizeKB > maxSizeKB) {
                        toast.error(`Image is too large. Please select an image smaller than ${maxSizeKB} KB`);
                        return;
                    }
                }
                
                toast.success(`Image compressed from ${fileSizeKB.toFixed(1)} KB to ${(finalFile.size / 1024).toFixed(1)} KB`);
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // For existing officer, ensure there's an image URL
        if (officer && !selectedFile && !officer.image_url) {
            toast.error('Please upload a photo');
            return;
        }
        ...
        try {
            ...
            if (!payload.name || !payload.designation || !payload.phone || !payload.email) {
                throw new Error('Name, designation, phone, and email are required fields');
            }
            ...
            console.log('Submitting placement officer:', {
                method,
                payload,
                isUpdate: !!officer
            });
            ...
            if (!response.ok) {
                let errorMessage = `Failed to ${officer ? 'update' : 'add'} placement officer`;
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <p className="text-xs text-gray-500">
                                        ...
                                        <br />
                                        <span className="text-blue-600">Large images will be automatically compressed</span>
                                    </p>
                                </div>
