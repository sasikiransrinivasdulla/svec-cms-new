...
export default function PlacementTeamPage() {
    ...
    // Verify team member exists in database
    const verifyTeamMemberExists = async (id: number): Promise<boolean> => {
        try {
            const response = await fetch('/api/placement/team');
            if (response.ok) {
                const data = await response.json();
                const memberExists = data.some((member: TeamMember) => member.id === id);
                console.log(`Team member ${id} exists in database:`, memberExists);
                return memberExists;
            }
            return false;
        } catch (error) {
            console.error('Error verifying team member existence:', error);
            return false;
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // If editing, get existing image URL and validate member exists
            if (editingId) {
                console.log('Current teamMembers state:', teamMembers);
                console.log('Looking for team member with ID:', editingId, 'Type:', typeof editingId);
                
                let member = teamMembers.find((m) => {
                    console.log('Comparing:', m.id, 'vs', editingId, 'Types:', typeof m.id, typeof editingId);
                    // Handle both string and number comparisons
                    return m.id === editingId || Number(m.id) === Number(editingId);
                });
                
                // If member not found in local state, try to fetch fresh data
                if (!member) {
                    console.warn(`Team member with ID ${editingId} not found in local state. Refreshing data...`);
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            member = freshData.find((m: any) => m.id === editingId || Number(m.id) === Number(editingId));
                            
                            if (member) {
                                // Update local state with fresh data
                                setTeamMembers(freshData);
                                console.log('Found team member in fresh data:', member);
                            } else {
                                throw new Error(`Team member with ID ${editingId} no longer exists in the database.`);
                            }
                        } else {
                        ...
                        throw new Error(`Team member with ID ${editingId} not found. Please refresh the page manually and try again.`);
                    }
                }
                
                imageUrl = member.image_url || '';
                console.log('Using team member for editing:', member);
            }
            ...
            if (editingId && (typeof editingId !== 'number' || editingId <= 0)) {
                throw new Error(`Invalid team member ID: ${editingId}`);
            }

            // Verify team member exists in database before update
            if (editingId) {
                const memberExists = await verifyTeamMemberExists(editingId);
                if (!memberExists) {
                    throw new Error(`Team member with ID ${editingId} no longer exists in the database. Please refresh the page.`);
                }
            }
            ...
            if (!response.ok) {
                let errorMessage = `Failed to ${editingId ? 'update' : 'add'} team member`;
