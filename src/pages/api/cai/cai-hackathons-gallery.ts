...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    if (req.method === 'GET') {
      // Query from cai_hackathons_gallery table
      ...
      const hackathonsGallery = Object.values(groupedByYear).map((item: any) => ({
        ...item,
      ...
      const result: any = await executeQuery(
        "INSERT INTO cai_hackathons_gallery (dept, category, academic_year, title, gallery) VALUES (?, 'hackathon', ?, ?, ?)",
      ...
      await executeQuery(
        "UPDATE cai_hackathons_gallery SET dept = ?, academic_year = ?, title = ?, gallery = ? WHERE id = ?",
