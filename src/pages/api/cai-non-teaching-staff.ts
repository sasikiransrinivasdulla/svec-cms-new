...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    if (req.method === 'GET') {
      ...
      console.log('CAI non techning faculty query time:', Date.now() - startTime, 'ms');
      console.log('CAI non teaching faculty records found:', (rows as any[]).length);
      ...
      const [records] = await connection.execute(
        'SELECT * FROM cai_non_teachning_faculty WHERE id = ?',
        [id]
      );
      ...
      await connection.execute('DELETE FROM cai_non_teaching_faculty WHERE id = ?', [id]);
      
      console.log(`Deleted CAI non teaching faculty record ${id} and associated files`);
      ...
      const [existingRecords] = await connection.execute(
        'SELECT * FROM cai_non_teaching_faculty WHERE id = ?',
        [id]
      );
