  ...
  try {
    if (req.method === 'GET') {
      ...
      const teaching = (await executeQuery(
        "SELECT *, 'teaching' as faculty_type FROM ect_faculty ORDER BY id ASC"
      )) as any[];
      ...
      try {
        technical = (await executeQuery(
          "SELECT *, 'technical' as faculty_type FROM ect_technical_faculty ORDER BY id ASC"
        )) as any[];
      } catch (e) {
        technical = [];
      }
      ...
      try {
        nonTeaching = (await executeQuery(
          "SELECT *, 'non_teaching' as faculty_type FROM ect_non_teaching_faculty ORDER BY id ASC"
        )) as any[];
      } catch (e) {
        nonTeaching = [];
      }
      ...
      const allFaculty = [...teaching, ...technical, ...nonTeaching];
